...
namespace aspect
{
  namespace Assemblers
  {
    ...
    ApplyStabilization<dim>::ApplyStabilization(const double stabilization_theta)
      :
      surface_stabilization_theta(stabilization_theta)
    {}
             ...
             internal::Assembly::CopyData::CopyDataBase<dim>      &data_base) const
    {
      ...
      if (this->get_parameters().include_melt_transport)
        {
          this->get_melt_handler().apply_free_surface_stabilization_with_melt (surface_stabilization_theta,
                                                                               scratch.cell,
                                                                               scratch,
                                                                               data);
          return;
  ...
  namespace MeshDeformation
  {
                                          ...
                                          aspect::Assemblers::Manager<dim> &assemblers) const
    {
      aspect::Assemblers::ApplyStabilization<dim> *surface_stabilization
        = new aspect::Assemblers::ApplyStabilization<dim>(surface_stabilization_theta);
    ...
    void MeshDeformationHandler<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection ("Mesh deformation");
      {
        surface_stabilization_theta = prm.get_double("Surface stabilization theta");
